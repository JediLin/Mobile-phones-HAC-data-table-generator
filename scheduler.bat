...
IF NOT EXIST "silent_update.bat" (
  ...
  pause > nul
  start "" "https://github.com/JediLin/Mobile-phones-HAC-data-table-generator/releases/latest"
  exit
)
IF NOT EXIST "mobile-accessibility-phones.xsl" (
  IF %colorSupport%==Yes (
    echo [31m⚠[0m  缺少 [32mmobile-accessibility-phones.xsl[0m 檔案，無法處理資料表。請把 mobile-accessibility-phones.xsl 跟 silent_update.bat 放在同一個資料夾內，然後再試一次。請按任意鍵結束……
  ) ELSE (
    echo 錯誤：缺少 mobile-accessibility-phones.xsl 檔案，無法處理資料表。請把 mobile-accessibility-phones.xsl 跟 silent_update.bat 放在同一個資料夾內，然後再試一次。請按任意鍵結束……
  )
  pause > nul
  exit
)
...
IF NOT EXIST "mobile-accessibility-phones.html" (
  IF %colorSupport%==Yes (
    echo [36m🔰[0m 未發現 [32mmobile-accessibility-phones.html[0m 檔案，無法確認是否已能正確處理資料表。現在即將執行 [33mupdate.bat[0m，請在確認成功後再嘗試加入排程自動執行。
  ) ELSE (
    echo 未發現 mobile-accessibility-phones.html 檔案，無法確認是否已能正確處理資料表。現在即將執行 update.bat，請在確認成功後再嘗試加入排程自動執行。
  )
  echo.
  echo 請按任意鍵繼續……
  pause > nul
...
echo.   cmd /C start "自動更新 HAC 資料表" /D "%cd%" "%~dp0silent_update.bat" 
echo.
IF %colorSupport%==Yes (
...
:doupdate
echo.
schtasks /Create /SC MONTHLY /MO SECOND /D THU /ST 14:30 /TN "自動更新 HAC 資料表" /TR "cmd /C start '自動更新 HAC 資料表' /D '%cd%' '%~dp0silent_update.bat'" /F
echo.
echo 設定完成。
echo.
...
echo.
IF %colorSupport%==Yes (
  echo [31m⚠[0m  警告：本操作將刪除任何名稱為「[32m自動更新 HAC 資料表[0m」的排程項目。
) ELSE (
  echo 警告：本操作將刪除任何名稱為「自動更新 HAC 資料表」的排程項目。
)
echo.
...
:doremove
echo.
schtasks /Delete /TN "自動更新 HAC 資料表" /F
echo.
echo 排程已刪除。
echo.
